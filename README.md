...
Перед первым запуском скопировать `.env.example` в `.env.local` (сам `.env.local` в `.gitignore` и в репозиторий не попадает):

```bash
cp .env.example .env.local
```

Там один параметр — `APP_URL=http://localhost:3000`, он нужен серверу для собственного fetch к `/api/meetings`. Страница `/chat` помечена `export const dynamic = "force-dynamic"`, поэтому рендерится на каждый запрос, а не один раз при сборке — иначе при отсутствии `APP_URL` в момент билда (например, в CI) ошибка получения встреч намертво запеклась бы в статический HTML.
